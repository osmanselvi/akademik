...
function get_image_url($filename) {
    ...
    // Try makaleler directory (for legacy PDFs)
    if (file_exists(publicPath('makaleler/' . $filename))) {
        return '/makaleler/' . $filename;
    }
    
    // Fallback to images directory (default behavior)
...
function setting($key, $default = null) {
    ...
    if ($settings === null) {
        try {
            $stmt = $baglanti->query("SELECT * FROM ayarlar LIMIT 1");
...
function logger($action, $details = '') {
    \App\Helpers\Logger::log($action, $details);
}
