...
$breadcrumbs = [
    ...
    ['text' => 'Üye Düzenle', 'url' => '/admin/kurul/edit/' . $member->id]
];
?>
            ...
            <div class="card border-0 shadow-sm rounded-4" data-aos="fade-up">
                <div class="card-header bg-white border-0 p-4 d-flex justify-content-between align-items-center">
                    <h4 class="fw-bold mb-0">
                        <i class="bi bi-pencil-square text-primary me-2"></i> Kurul Üyesi Düzenle
                    </h4>
                    <span class="text-muted small">ID: #<?= $member->id ?></span>
                </div>
                <div class="card-body p-4">
                    <form action="/admin/kurul/update/<?= $member->id ?>" method="POST" class="needs-validation" novalidate>
                        <?= csrf_field() ?>
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="unvan" class="form-select bg-light border-0 py-2 rounded-3" required>
                                    ...
                                    <?php foreach($unvanlar as $u): ?>
                                        <option value="<?= $u->id ?>" <?= $member->unvan == $u->id ? 'selected' : '' ?>>
                                            <?= e($u->unvan) ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="kurul_select" class="form-select bg-light border-0 py-2 rounded-3" required>
                                    <?php foreach($kurullar as $k): ?>
                                        <option value="<?= $k->id ?>" <?= $member->kurul_id == $k->id ? 'selected' : '' ?>>
                                            <?= e($k->kurul) ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="kurul_gorev" id="gorev_select" class="form-select bg-light border-0 py-2 rounded-3" required>
                                    <?php foreach($gorevler as $g): ?>
                                        <?php if($g->kurul == $member->kurul_id): ?>
                                            <option value="<?= $g->id ?>" <?= $member->kurul_gorev == $g->id ? 'selected' : '' ?>>
                                                <?= e($g->kurul_gorev) ?>
                                            </option>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
                            ...
                            <div class="col-12">
                                <div class="form-check form-switch bg-light p-3 rounded-4 d-flex align-items-center">
                                    <input class="form-check-input ms-0 me-3 mt-0" type="checkbox" name="is_approved" id="isApproved" <?= $member->is_approved ? 'checked' : '' ?>>
                                    <div>
