...
class MakaleController extends BaseController {
    ...
    public function show($id) {
        ...
        // Increment View Count (Session based check to prevent spam)
        if (!isset($_SESSION['viewed_article_' . $id])) {
            $this->makaleModel->incrementView($id);
            $_SESSION['viewed_article_' . $id] = true;
        }
    ...
    public function download($id) {
        $makale = $this->makaleModel->find($id);
        
        if (!$makale || empty($makale->dosya)) {
            $this->redirect('/', 'Dosya bulunamadı.');
        }

        // Increment Download Count
        $this->makaleModel->incrementDownload($id);

        $fileUrl = get_image_url($makale->dosya);
